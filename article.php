<?php
    ...
    $sql="SELECT * FROM blog_objave WHERE b_id='" . $_GET["b_id"] . "'";
...
<?php
  ...
  <p class="slikaArticle"><?php echo '<img src="data:image/jpeg;base64,'.base64_encode($row['slika'] ).'" height="400" width="260"/>';?></p>
    ...
    <a class="nazadArticle" href="index1.php">Nazad na vijesti</a>
   </div>
  <?php
	$i++;
    }
...
if(isset($_SESSION['username'])){
    ...
    <div class="komentar">
      <form class="form-group" action="procesi/komentari.blog.proces.php" method="POST" >

      <input  type="hidden" name="blog-id-comment" value="'. $_GET['b_id'].'" readonly >
      <input  type="hidden" name="username-id-comment" value="'.$_SESSION['username_id'].'" readonly >
       ...
       <button type="submit" name="comment-blog-button" class="dugme">Podijeli komentar</button>
      </form>
      </div>';
...
<?php
  ...
  $find_comments = mysqli_query($connect,"SELECT * FROM komentari_blog WHERE b_id=$blog_id ORDER BY datum desc");
  ...
  while($row =mysqli_fetch_assoc($find_comments) ){
    ...
    if(isset($_SESSION['username_id'])){
     ...
     if($row['u_id']==$user_id){
        ...
        echo '<a class="btn" style="color:red;background-color:black;" href="izbrisiKomentarBlog.php?kb_id='.$row['kb_id'].'">Izbrisi</a>';
